...
/*   Updated: 2017/10/17 12:10:17 by afeuerst         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "purplegreensh.h"

static inline void			foreground_task_setstd(t_task *const task, int *const fd)
{
	close(fd[0]);
...
static void __dead2			foreground_taskpipe_execute(t_dispatch *const dispatch,
							t_task *const task, int *const fd)
{
	t_cmd					*cmd;
	static const t_execute	exec_functions[] =
	{
		[CMD_OK] = exec_pure,
		[CMD_RED] = exec_red,
	...
	while (cmd)
	{
		...
		exec_functions[cmd->type](dispatch, task, cmd);
...
static void __dead2			foreground_taskpipe_execlast(t_dispatch *const dispatch, t_task *const task)
{
	t_cmd					*cmd;
	static const t_execute	exec_functions[] =
	{
